...
router.post('/', passport.authenticate('jwt', { session: false }), (req, res) => {
  ...
  // check validation
  if(!isValid){
    // if any error send 400 status code with errors object
  ...
  Profile.findOne({ user: req.user.id })
    .then(profile => {
      Post.findById(req.params.id)
        .then(post => {
          if(post.likes.filter(like => like.user.toString() === req.user.id).length === 0){
            return res.status(400).json({ notliked: 'You have yet liked this post' });
          }
...
router.delete('/comment/:id/:comment_id', passport.authenticate('jwt', {session:false}), (req, res) => {
  ...
  Post.findById(req.params.id)
    .then(post => {
      //check to see if comment exisit
      if(post.comments.filter(comment => comment._id.toString() === req.params.comment_id).length === 0){
        return res.status(404).json({ commentnotexisits: 'Comment does not exisit'})
      }
